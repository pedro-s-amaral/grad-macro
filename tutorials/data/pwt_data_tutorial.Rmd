...
The PWT is a database with information on relative levels of income, output, input, and productivity, covering 183 countries between 1950 and 2019. You can see more details [at the Groningen's center website](https://www.rug.nl/ggdc/productivity/pwt/).

Let's start by installing and loading the package `pwt10`. This package contains all the data in the PWT. You only need to install the package once (that is why have it commented out), but you need to load it at the start of every session.
...
By running the command `data("pwt10.0")` you have access to an R data frame (also called `pwt10.0`) with all the information in the data base. Take your time to examine the database py clicking on it in the \emph{Environment} box on the top-right side. 

```{r}
data("pwt10.0")
```

Part of what you need to do in the assignment is to filter out countries that do not have available data for some series. Let's say you needed to compute the capital-output ratio for countries that strictly positive levels of capital and output in 1970. You can use piping `%>%` to do this, but you need to install the package `dplyr`
...
To create a histogram of the average capital to output ratio you need to install and load the graphics package `ggplot2`. 

```{r, fig.alt="Chart containg a histogram of the average capital output-ratio for different countries between 1970 and 2019."}
...
```{r, fig.alt="Chart containg a histogram of the GDP per worker for different countries in 1970."}
...
Now we can regress the capital output ratio on GDP per worker using the function `lm()`. Don't worry if for some countries GDP per worker is not available, they will be automatically excluded from the regression.
...
```{r, fig.alt="Chart containg a scatterplot of the average capital output-ratio for different countries between 1970 and 2019 and their GDP per worker in 1970."}
# chart
