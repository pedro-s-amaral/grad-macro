...
The syntax for the optimization is `optim(par = <initial parameter>, fn = <obj. function>, method = <opt. routine>)`. The first argument is an initial guess. The second argument is the function to be optimized, and the third argument is the optimization method to be used. See [the documentation](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/optim) for the different methods and a general description of the function. By default, `optim()` minimizes the objective function.
...
```{r, fig.alt="A plot of a quadratic function that is u-shaped emphasizing its minimum point"}
library(ggplot2)
...
```{r, fig.alt="A plot of a quadratic function that is inverse u-shaped emphasizing its maximum point"}
...
To check the second derivative we need to compute the Hessian matrix evaluated at the optimal point $\left| {\begin{array}{cc} \frac{\partial ^2f}{\partial x^2} & \frac{\partial ^2f}{\partial x \partial y} \\  \frac{\partial ^2f}{\partial y \partial x} & \frac{\partial ^2f}{\partial y^2} \end{array} } \right|_{x_0,y_0}.$
...
Notice how the two eigenvalues are positive, so matrix $M$ is positive definite and our optimizer is a minimum. Of course we could have saved ourselves a lot of work by letting R compute the eigenvalues directly.
...
```{r, fig.alt="A three dimensional plot of a surface, emphasizing that it has a minumum and not a maximum"}
# Load libraries
library(ggplot2)
library(plotly)
